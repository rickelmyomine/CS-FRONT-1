...
from providers.db_mock import TelemetriaRepository # NOVO: Importação do simulador de estado
import plotly.express as px

def render_dados():
    st.markdown("### 📊 Monitoramento de Sensores e Telemetria")
    
    # --- NOVO: Exibir a localização selecionada na barra lateral ---
    ...
    # --- NOVO: Indicadores de Alerta e Status (Requisito 4) ---
...
# --- NOVO: Requisito 5 - Integração de Cadastro Visual ---
    with st.expander("📷 Visualizar Placa do Ativo (Extração via Visão Computacional)"):
        ...
        with col_img:
            ...
            # No futuro, isso pode vir de uma pasta local como 'assets/placa_motor.png' ou banco de dados
        ...
        with col_info:
            ...
            # Aqui podemos simular os dados que vieram do state de equipamentos
    ...
    with col_principal:
        ...
        with tab_grafico:
            ...
            # --- NOVO: Requisitos Técnicos - Gráfico Plotly com UX avançada ---
            # O Plotly atende ao requisito de rastreabilidade (zoom, hover, exportação)
            fig = px.line(
                df_atual, 
                x='Tempo (s)', 
                y='RPM_Convertido', 
                markers=True, # Adiciona bolinhas em cada ponto para facilitar a leitura
                title="Evolução do RPM ao Longo do Tempo (Histórico da Sprint 1)",
                labels={'RPM_Convertido': 'RPM', 'Tempo (s)': 'Tempo em Segundos'}
            )
            
            # Ajuste de UI/UX: Customizando o layout para ficar mais limpo
            fig.update_layout(
                hovermode="x unified", # Mostra uma linha guia ao passar o mouse
